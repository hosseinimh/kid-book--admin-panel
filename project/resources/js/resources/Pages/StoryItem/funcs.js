...
import { storyItemsPage as strings } from "../../../constants/strings";
...
const setStoryId = (storyId) => {
    _storyId = !isNaN(storyId) && storyId > 0 ? storyId : 0;
};
...
const fillForm = async (data = null) => {
    ...
    await fetchStoryItems(data);

    _dispatch(setLoadingAction(false));
};

const fetchStoryItems = async (data = null) => {
    let result = await _entity.getAll(_storyId);
